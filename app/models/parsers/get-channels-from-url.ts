...
import { booleanFilter, filterUnique } from "../../utils";
import { ChannelErrors } from "../channel.server";
...
export async function getChannelsFromUrl(
  ...
  const response = await fetchChannel(url, abortSignal).catch(() => {
...
const fetchChannel = cached("fetch-channel", {
