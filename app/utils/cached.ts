...
export function cached<T extends unknown[], U>(
  ...
  const cachedFn = async (...args: T) => {
    ...
    const cachedResult = Cache.get(name, cacheKey);
    ...
    if (params.doNotCache?.(result) !== true) {
      Cache.set(name, cacheKey, { value: result, ttl: params.ttl });
    }
...
class Cache {
  ...
  >();
  static get(fnName: string, key: string) {
    const invocationMap = this.fnMap.get(fnName);
...
if (import.meta.vitest) {
  ...
  test("cache happy path", async () => {
    ...
    expect(await cachedFn("val_a")).toBe(0);
    resultStore["val_a"] = 1;
    ...
    expect(await cachedFn("val_b")).toBe(5);
  });
  ...
  test("do not cache", async () => {
    ...
    expect(await cachedFn("val")).toBe(null);
    resultStore["val"] = 1;
    // Cache should return the new val value
    expect(await cachedFn("val")).toBe(1);
  });
}
