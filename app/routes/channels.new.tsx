...
const inputs = enumerate(["channel-url", "redirect"]);
...
export const action = async ({
  ...
  const inputChannelHref = formData.get(inputs["channel-url"]);
  ...
  try {
    ...
    invariant(channelResponse);
  } catch (_) {
    return {
      errors: { fetch: `Could not load RSS feed from "${channelUrl.origin}"` },
    };
  }
  ...
  if (singleFeedResponse) {
    ...
    if (formData.get(inputs["redirect"]) !== String(false)) {
      throw redirect(
  ...
  return {
    inputUrl: channelUrl,
    feeds:
      channelResponse?.map((foundChannel) => ({
      ...
      <div className="flex max-w-xl flex-col gap-4">
        <Form method={"PUT"} className={"flex flex-col gap-4"}>
          ...
          <SubmitSection
            isSubmitting={
              isSaving &&
              navigation.formData?.get(inputs["redirect"]) !== String(false)
            }
                    ...
                    {channel.channelId ? (
                      ...
                      <Form method={"PUT"}>
                        ...
                        <FormButton
                          ...
                          className="h-fit rounded border-current bg-rose-50 p-1 px-2 text-rose-700 active:bg-rose-100"
                        >
